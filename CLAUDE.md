...
Phase 4 has since fixed the save cascade it inherited: `SqliteStoryRepository.SaveAsync` upserts scenes by id and deletes only the scenes a story has dropped, so `editor_scene_layout` and the D6 `extension_*` slot survive a save, and node positions have their own Application port (`ISceneLayoutStore`, reached as `IStoryWorkspace.Layout`) that never raises `Changed`. Since M21 the canvas writes a scene's position when a drag ends, and the map slides and zooms under a viewport (`CanvasViewport`, `CanvasInteraction`, and a thin JS shim under `Components/Canvas/`), with "Show whole story" and "Actual size" in the toolbar.
...
- **Component styles live in the global `wwwroot/app.css`, not in per-component `.razor.css` files**, although `overlays/frontend-blazor.md` puts component-scoped styles in `<Name>.razor.css`. The editor's components share one token system and one visual vocabulary (eyebrow headings, severity rows, text buttons, the ribbon bookmark) applied by role across components; splitting it per component would repeat the shared rules in every file and push them through `::deep` wherever one component styles what another renders. The exception covers the Phase 3 chrome only. The layout components (`MainLayout`, `ReconnectModal`) keep a `.razor.css` for host-provided markup they alone own, and the canvas components under `Components/Canvas/` (`StoryCanvas`, `SceneNode`, `SceneEdge`, `SceneEdgeLabel`) follow the overlay with a scoped `.razor.css` each: their SVG strokes, markers, and halos are private to the canvas and share nothing with the dock but tokens (decided in M20; see `docs/design/0002-visual-direction.md` §10.7). A new component whose rules are private to it gets its own `.razor.css`; a rule shared by role across components goes in `app.css`.
- **Argument-guard tests pass `null!` without a comment**, although `core/coding-standards.md` asks for a comment justifying every `!`. In a test named for its guard (`Should_Throw_When_…IsNull`, an `Assert.Throws<ArgumentNullException>` around the call), passing null *is* the test, and a comment on each would repeat its name. The exception covers exactly that idiom in test projects; any other `!`, in tests or production code, still needs its comment. Decided in the review of the M22 PR.
