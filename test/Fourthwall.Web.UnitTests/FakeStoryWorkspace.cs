...
public sealed class FakeStoryWorkspace : IStoryWorkspace
{
    ...
    // The stores only exist while a story is open, matching the real workspace.
    public IAssetStore? Assets => Current is null ? null : AssetStore;

    public FakeAssetStore AssetStore { get; } = new();

    public ISceneLayoutStore? Layout => Current is null ? null : LayoutStore;

    public FakeSceneLayoutStore LayoutStore { get; } = new();
